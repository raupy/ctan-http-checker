...
public class UpdateChecker {
	 ...
	 */
	public static ArrayList<String> initCtanArray() throws IOException {
		ArrayList<String> ctanMirrors = new ArrayList<String>();
		BufferedReader in = new BufferedReader(new FileReader("C:\\Users\\lil72389\\eclipse-workspace\\LaTexCrawler\\src\\CtanMirrors.txt"));
        String inputLine = "";
        while ((inputLine = in.readLine()) != null)
	...
	public static ArrayList<ArrayList<String>> checkForUpdates(ArrayList<String> ctanMirrors, ArrayList<ArrayList<String>> updateList) {
		for(int i = 0; i < ctanMirrors.size(); i++) {
			...
			if(!timestamp.equals(lastUpdate)) mirrorI.add(lastUpdate);
		}
		return updateList;
	}
	...
	public static void writeToFile(ArrayList<ArrayList<String>> updateList, String version) {
			...
			try {
				writer = new BufferedWriter(new FileWriter("C:\\Users\\lil72389\\eclipse-workspace\\LaTexCrawler\\src\\UpdateFile" + version + ".txt"));
			} catch (IOException e) {
				e.printStackTrace();
			}
			try {
	...
	public static void main(String[] args)  {
		...
		if(ctanMirrors != null && ctanMirrors.size() > 0) {
			...
			checkAndSleep(ctanMirrors, updateList, LocalDateTime.now().plusDays(1), 1800000); //check for updates, wait 30 minutes
		}
	}
