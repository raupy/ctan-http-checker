package latexDiffCrawler;

public class Constants {
	public static String ROOT_DIR = "C:\\Users\\Lilli\\eclipse-workspace\\LaTexCrawler\\";
	public static String fileWithCtanMirrors = ROOT_DIR + "CtanMirrorsBig4.txt";
	public static String mirrorData = ROOT_DIR + "mirrorData.txt";
	public static String MIRROR_DIR = ROOT_DIR + "repo\\mirrors";
	public static String MASTER_DIR = ROOT_DIR + "repo\\master";
	public static String MASTER_DIFFICULT_FILES_DIR = ROOT_DIR + "repo\\masterDifficultFiles";
	public static String DIFFICULT_FILES = ROOT_DIR + "difficultFiles.txt";
	public static String DANTE = "http://dante.ctan.org/tex-archive/";
	public static String MASTER_HASHES = ROOT_DIR + "masterHashes.txt";
	public static String RSCYNC_LOG = ROOT_DIR + "rsync.log";
}
